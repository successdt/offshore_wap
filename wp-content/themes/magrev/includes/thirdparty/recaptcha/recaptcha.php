...
$recaptcha_opt = $GLOBALS['_webnukes']->get_options('sub_APIs');
...
function recaptcha_wp_hash_comment ($id)
{
	...
	if (function_exists('wp_hash') ) {
		...
		return md5 (RECAPTCHA_WP_HASH_COMMENT . $recaptcha_opt['recaptcha_private'] . $id);
	}
}
...
function recaptcha_wp_get_html ($recaptcha_error) 
{
	...
	return recaptcha_get_html($recaptcha_opt['recaptcha_public'], $recaptcha_error);
}
...
if( $GLOBALS['_webnukes']->get_options('sub_general', 'comment_status') == 'on' )
...
function recaptcha_wp_check_comment($comment_data) {
	...
	$status = $GLOBALS['_webnukes']->get_options('sub_general_settings');
	...
	if (recaptcha_wp_show_captcha_for_comment ()) {
		if ( $comment_data['comment_type'] == '' ) { // Do not check trackbacks/pingbacks
			...
			$recaptcha_response = recaptcha_check_answer ($recaptcha_opt ['recaptcha_private'], $_SERVER['REMOTE_ADDR'], $challenge, $response);
