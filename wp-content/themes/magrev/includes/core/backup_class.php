...
class fw_backup_class
{
	...
	function theme_options_import($file = '')
	{
		...
		$res = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."terms WHERE ".$wpdb->prefix."terms.slug = 'top-bar-menu' OR ".$wpdb->prefix."terms.slug = 'nav-bar-menu'");
		
		if($res){
			$nav_menu = array();

			$nav_menu['nav_menu_locations']['top-bar'] = $res[1]->term_id;
			$nav_menu['nav_menu_locations']['nav-menu'] = $res[0]->term_id;
	...
	function theme_options_export($file = '')
	{
		...
		foreach($options as $k=>$v)
		{
			if(key($v) == 'SUB')
			{
				...
				{
					$option_name = 'sub_'.$vk;
					if($settings = $this->_webnukes->get_options($option_name))
					{
		...
		$data['sidebars_list'] = get_option('magrev_sidebars_list');
