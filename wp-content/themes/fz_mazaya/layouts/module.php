...
if ($style == '') {
    if ($module->position == 'newflash') $style = 'blank';
	if ($module->position == 'top-a') $style = 'simple';
	if ($module->position == 'top-b') $style = 'box';
	if ($module->position == 'bottom-a') $style = 'blank';
	if ($module->position == 'bottom-b') $style = 'blank';
...
if (in_array($module->position, array('headerbar', 'toolbar-r' ,'toolbar-l', 'footer','newflash'))) {
	$style = '';
	$showtitle = 0;
}
...
switch ($style) {
 ...
 case 'simple':
	 ...
	 case 'dottedline':
		$template		= 'box-simple';
		$style			= 'mod-'.$style;
		$style			.= ($color) ? ' mod-dottedline-'.$color : '';
		$split_color	= 1;
		$subtitle		= 1;
		$title_template = '<h3 class="module-title">%s</h3>';
		break;
...
if ($module->menu) {
	...
	} else if (in_array($module->position, array('menu', 'toolbar-l'))) {
		$renderer = 'dropdown';
	} else if (in_array($module->position, array('toolbar-l', 'toolbar-r', 'footer','newflash'))) {
		$renderer = 'default';
	} else {
		$renderer = 'accordion';
	}
