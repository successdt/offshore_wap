...
/***************** Breadcrum Navi *****************/
if (!function_exists('cm_breadcrumb_nav')) {
    function cm_breadcrumb_nav() {
        if ( !is_home() || !is_front_page() ) {
            if ( is_category() ) {
		single_cat_title() ._e("</a>");
            } elseif ( is_single() ) {
		the_category() ._e("<span>&nbsp;>&nbsp;</span> <a>") .the_title() ._e("</a>");
            } elseif ( is_page() ) {
		_e("<a href=''>") ._e( the_title() ) ._e("</a>");
            } elseif (is_page() && $post->post_parent ) {
		_e( get_the_title($post->post_parent) );
		_e(" > <a>") ._e( the_title() ) ._e("</a>");
	    } elseif ( is_search() ) {
		_e("<a><strong>Kết quả tìm kiếm cho:</strong> ") . the_search_query() ._e("</a>");
            }
        }
    }
}?>
