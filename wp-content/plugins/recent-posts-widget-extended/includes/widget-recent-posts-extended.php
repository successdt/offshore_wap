				...
				<?php foreach ( $rpwewidget as $post ) : setup_postdata( $post ); ?>
					...
					<li class="rpwe-clearfix clearfix cl">
						...
						<?php if ( $thumb == true ) { ?>
							...
							<?php } 
							elseif (function_exists('catch_that_image') && catch_that_image(get_the_content())) {  ?>
								<?php echo '<a href="' . esc_url( get_permalink() ) . '" rel="bookmark"><img width="' . $thumb_width .'" height="' . $thumb_height . '" class="' . $thumb_align . ' rpwe-thumb" src="' . catch_that_image(get_the_content()) . '" alt="' . esc_attr( get_the_title() ) . '"></a>'; ?>
							<?php }
							else { ?>				
